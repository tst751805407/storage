<?php if (!defined('THINK_PATH')) exit();?><div class="bjui-pageContent tableContent">
    <form action="/Storage/index.php?s=/Home/Chanpin/edit/navTabId/<?php echo CONTROLLER_NAME;?>" class="pageForm" data-toggle="validate">
		<input type="hidden" name="id" value="<?php echo ($id); ?>">
        <div class="pageFormContent" data-layout-h="0">
          <table class="table table-condensed table-hover" width="100%">
           <tbody>
		   <tr><td><label for='bianhao_input' class='control-label x85'>产品编号:</label><input type='text' id='bianhao' name='bianhao' data-rule='required;' size='20' readonly  value='<?php echo ($Rs["bianhao"]); ?>'  ></td><td><label for='title_input' class='control-label x85'>产品名称:</label><input type='text' id='title' name='title' data-rule='required;' size='20'   value='<?php echo ($Rs["title"]); ?>'  ></td></tr>
<tr><td><label for='fenlei_input' class='control-label x85'>
    产品分类:</label><select name='fenlei'  data-toggle='selectpicker' ><option value=''>请选择</option><option value='变速箱类'>变速箱类</option><option value='发动机类'>发动机类</option><option value='汽车零件'>汽车零件</option><option value='配套软件'>配套软件</option></select></td><td><label for='type_input' class='control-label x85'>
    产品规格:</label><input type='text' id='type' name='type'  size='20'   value='<?php echo ($Rs["type"]); ?>'  ></td></tr>
