...
<form id="pagerForm" data-toggle="ajaxsearch" action="/Storage/index.php?s=/Home/Quxiang" method="post">
        ...
        <div class="bjui-searchBar">
            <label>关键词：</label><input type="text" value="<?php echo ($_REQUEST['keys']); ?>" name="keys" class="form-control" size="12" />
             ...
             <button type="submit"  class="btn-default" data-icon="search">查询</button>
			  ...
			  <?php if(display(CONTROLLER_NAME.'/add')){ ?><span style="float:right;margin-right:5px;"><a href="/Storage/index.php?s=/Home/Quxiang/add/navTabId/<?php echo CONTROLLER_NAME;?>" class="btn btn-green" data-toggle="dialog" data-width="900" data-height="500" data-id="dialog-mask" data-mask="true" data-icon="plus">新增</a></span><?php } ?>
		</div> 
</form>
...
<td><?php echo (substr($v["addtime"],0,10)); ?></td>
		   ...
		   <?php if(display(CONTROLLER_NAME.'/view')){ ?><td><a href="/Storage/index.php?s=/Home/Quxiang/view/id/<?php echo ($v['id']); ?>/navTabId/<?php echo CONTROLLER_NAME;?>"  data-toggle="dialog" data-width="900" data-height="500" data-id="dialog-mask" data-mask="true" >详细</a></td><?php } ?>
		   <!--<?php if(display(CONTROLLER_NAME.'/lock')){ ?><td><a href="/Storage/index.php?s=/Home/Quxiang/lock/id/<?php echo ($v['id']); ?>/navTabId/<?php echo CONTROLLER_NAME;?>" data-toggle="doajax" data-confirm-msg="确定要操作吗？"><?php if($v["status"] == 1 ): ?><img src="/Storage/Public/images/ok.gif" border="0"/><?php else: ?><img src="/Storage/Public/images/locked.gif" border="0"/><?php endif; ?></a></td><?php } ?>-->
		   <?php if(display(CONTROLLER_NAME.'/edit')){ ?><td> <a href="/Storage/index.php?s=/Home/Quxiang/edit/id/<?php echo ($v['id']); ?>/navTabId/<?php echo CONTROLLER_NAME;?>"   class="btn btn-green btn-sm" data-toggle="dialog" data-width="900" data-height="500" data-id="dialog-mask" data-mask="true" >编辑</a></td><?php } ?>
		   </td>
